...
// still green.
//
// Provenance: these tables were cross-checked against a second, independent
// implementation of each language's naming rules — a standalone script that
// does not import or read js/data/number-words.js at all, so it cannot
// inherit a bug from it — encoding the same ones/tens/compose shape from
// first principles per language and comparing its output for 1..100 against
// every literal below. That script is throwaway verification tooling, not
// app or test code, so it is not committed; it lives only in a scratch
// working directory during review. It is deliberately not derived from this
// module's own output (which would make the pin tautological, the exact
// failure mode this comment exists to rule out) — it is a second,
// from-scratch encoding of the same linguistic rules, run once to confirm
// the two independent derivations agree, with zero mismatches found across
// all 500 forms at time of writing.
const FULL_EN = {
