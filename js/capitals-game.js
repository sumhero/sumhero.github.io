const CapitalsGame = {
    ...
    generateChoices(correctCountry, pool) {
        const correctCapital = getCapitalName(correctCountry);
        ...
        while (choices.length < 5 && others.length > 0) {
            ...
            const capital = getCapitalName(others[idx]);
    ...
    showExercise() {
        ...
        diceContainer.innerHTML =
            ...
            '<div class="geo-country-name">' + getCountryName(exercise.country) + '</div>';
    ...
    answer(value, btn) {
        ...
        const exercise = this.session.exercises[this.currentExercise];
        const isCorrect = value === getCapitalName(exercise.country);
